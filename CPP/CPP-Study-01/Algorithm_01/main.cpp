 ...
 * 
 * 依据指定容器的迭代器 start 和 迭代器 end 去遍历容器中的元素，把它们作为实参逐个输入至所指定的 一元仿函数或者函数指针 当中，最后会返回所指定的 一元仿函数或者函数指针
*/
 ...
 * 
 * 依据原始容器的迭代器 start 和 迭代器 end 去遍历原始容器中的元素，把它们作为实参逐个输入至所指定的 返回值和形参类型为容器中的元素类型的一元仿函数或者函数指针 unary_op 当中，并依照它的返回结果 [重新赋值] 给迭代器 start_des 所指向的目标容器中
