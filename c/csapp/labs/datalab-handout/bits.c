...
int isAsciiDigit(int x) {
   ...
   *
   *  其中，第 1 的情况判断 4~7bit 恒为 0010 的目的看似多余，其实他是为了过滤掉特殊的情况，如 0, TMax, TMin，并且，情况 1 和情况 23 必须同时为 true 才能够返回正确的结果，故这里使用了 位与 进行了连接，保证两种情况(情况1 和 情况23)同时为 true 时才返回正确结果，即: 
