...
0000000000400efc <phase_2>:
  ...
  400f02:	48 89 e6             	mov    %rsp,%rsi  # 咱存当前的栈指针的位移至 rsi.
																									# 这是要给 <read_six_numbers> 函数去使用吗？
																									# 去使用上一个栈帧中的数据？
  400f05:	e8 52 05 00 00       	callq  40145c <read_six_numbers>

# =====================================================================================
  400f0a:	83 3c 24 01          	cmpl   $0x1,(%rsp)           # 判断输入的第一个字符是否为 1
  400f0e:	74 20                	je     400f30 <phase_2+0x34> 
  400f10:	e8 25 05 00 00       	callq  40143a <explode_bomb>
  400f15:	eb 19                	jmp    400f30 <phase_2+0x34>

  400f17:	8b 43 fc             	mov    -0x4(%rbx),%eax       # 每一次循环的开始，读取指针 rbx 的上一个偏移的字符
  400f1a:	01 c0                	add    %eax,%eax						 # 上一个偏移的字符 * 2 如果等于 rbx 当前所指向的字符，才作为正确结果
  400f1c:	39 03                	cmp    %eax,(%rbx)
  400f1e:	74 05                	je     400f25 <phase_2+0x29>
  400f20:	e8 15 05 00 00       	callq  40143a <explode_bomb>

  400f25:	48 83 c3 04          	add    $0x4,%rbx             # 逻辑结束，开始添加 rbx 指针位移，并判断是否越界行为
  400f29:	48 39 eb             	cmp    %rbp,%rbx
  400f2c:	75 e9                	jne    400f17 <phase_2+0x1b>
  400f2e:	eb 0c                	jmp    400f3c <phase_2+0x40>

  400f30:	48 8d 5c 24 04       	lea    0x4(%rsp),%rbx        # 声明指针 rbx，其指向了每次循环时，下一个字符的偏移
  400f35:	48 8d 6c 24 18       	lea    0x18(%rsp),%rbp			 # 该指针为尾指针，用于判断 rbx 的偏移是否越界
# =====================================================================================
...
0000000000400f43 <phase_3>:
  ...
  400f51:	be cf 25 40 00       	mov    $0x4025cf,%esi # scan format string is: %d %d
  ...
  400f65:	e8 d0 04 00 00       	callq  40143a <explode_bomb>
																...
																# 这里应该是 switch 语义，执行的跳转表
																#  see: x/8xg 0x402470
  400f6a:	83 7c 24 08 07       	cmpl   $0x7,0x8(%rsp)
  400f6f:	77 3c                	ja     400fad <phase_3+0x6a> # 判断输入的数是否 < 8，因为跳转表仅有 8 个元素
...
000000000040100c <phase_4>:
  ...
  401024:	e8 c7 fb ff ff       	callq  400bf0 <__isoc99_sscanf@plt> # %d %d
...
# 0x7ffffffedc80: 0x0000000000603320      0x0000000000603310
# 0x7ffffffedc90: 0x0000000000603300      0x00000000006032f0
# 0x7ffffffedca0: 0x00000000006032e0      0x00000000006032d0
#
...
000000000401242 <secret_phase>:
  ...
  40125a:	48 89 c3             	mov    %rax,%rbx   # <strtol> return convert interage to rax
  40125d:	8d 40 ff             	lea    -0x1(%rax),%eax
  401260:	3d e8 03 00 00       	cmp    $0x3e8,%eax # 如果输入的字符 > 1000 则炸弹引爆
...
#		* rdi: source string
# return:
#   * rax: if 0, means the source string(rdi) is not a blank line, either nor.
...
# feature
#	 1. 每一次 phase 的开始都会读取一次字符串，并记录读取的次数
#		 * see: 0x603760 <num_input_strings>
#  2. 每一次读取，num_input_strings 都会自增 1，并且以 0x603780 为基础偏移 80 * num_input_strings
#  3. 每一次读取到的字符串都会判断该字符串是否为空行(blank_line)，该函数还会附带去除字符串末尾 '\n' 的作用
#		 * see: <blank_line>
#  4. 最后返回 rax, rdi，他们作为所读取字符串在内存中的起始地址
00000000004013f9 <skip>:
  ...
  401417:	be 50 00 00 00       	mov    $0x50,%esi
																...
																#  * 如果函数调用失败，则返回空指针，即 rax = 0x0
  40141c:	e8 5f f7 ff ff       	callq  400b80 <fgets@plt>

  401421:	48 89 c3             	mov    %rax,%rbx
  401424:	48 85 c0             	test   %rax,%rax # Check if empty input.
...
#                |0x00000004|
# 0x7ffffffedcac +----------+         
#								 |0x00000003| # how do this block stored the user input string?
# 0x7ffffffedca8 +----------+  00000       <-- rbx
#								 |0x00000002|
# 0x7ffffffedca4 +----------+         
#                |0x00000001|
# 0x7ffffffedca0 +----------+-------+ <-- rdx, rsi
#                |0x0000000000400f0a| # return rip
# 0x7ffffffedc98 +------------------+
#									callee stack frame
#                +------------------+
#                |0x0000000000000028|
...
000000000040145c <read_six_numbers>:
  ...
  40148f:	83 f8 05             	cmp    $0x5,%eax											# 如果輸入的字符數
...
#  1. 读取用户输入: 
#		 * see: 0x4014a7
#  2. 在读取的过程中(call <skip>)还会判断空白行之类的校验
#  3. 计算用户输入的长度，不满足则炸弹爆炸
#		 * see: 0x401552
#  4. 取出掉末尾的 \n 字符
#    * 
#  5. 字符串存储buffer的偏移量+1
#    * see: 0x4013fa ~ 0x401409
000000000040149e <read_line>:
  ...
  40153f:	48 c7 c1 ff ff ff ff 	mov    $0xffffffffffffffff,%rcx
																...
																# 计算字符串长度
																# the length of source string(rdi) is set to rcx
																#		see: https://blog.csdn.net/abc_670/article/details/79479364
  401546:	f2 ae                	repnz scas %es:(%rdi),%al
  401552401548:	48 f7 d1             	not    %rcx


  40154b:	48 83 e9 01          	sub    $0x1,%rcx # 为什么要长度要减1?

	# 如果所输入的字符串长度超过 0x4e，则报错
  ...
  40159a:	83 e9 01             	sub    $0x1,%ecx # 这里长度又减1了，但是
																								 # 通过观察可以发现，好像真实的字符串长度
																								 # 为调用完 repnz scas 后减2，应该是一个是 \0，然后还有一个是 \n
  ...
  4015ab:	c6 84 01 80 37 60 00 	movb   $0x0,0x603780(%rcx,%rax,1) # 这一段应该就是去除 \n 的
  4015b2:	00 

	# edx set to [40151f]，每一次 <read_line> 都要 +1？
...
00000000004015c4 <phase_defused>:
  ...
  015fa:	e8 f1 f5 ff ff       	callq  400bf0 <__isoc99_sscanf@plt>
  4015ff:	83 f8 03             	cmp    $0x3,%eax
  401602:	75 31                	jne    401635 <phase_defused+0x71>

  401604:	be 22 26 40 00       	mov    $0x402622,%esi # %esi: input string
  ...
  40163f:	48 8b 44 24 68       	mov    0x68(%rsp),%rax
	...
	# 对比开始时设置的金丝雀是否等于 rax，如果不等于则证明发生了栈溢出
	# 关于栈溢出攻击，see: csapp, 200
