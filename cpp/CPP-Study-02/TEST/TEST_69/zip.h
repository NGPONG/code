...
template<typename... T>
struct zip_itr_base : std::iterator<std::forward_iterator_tag, std::tuple<navit_val_type<T>...>> {
private:
...
public:
  struct iterator : zip_itr_base<T...> {
    iterator(navit_itr_type<T>... iters)
      : zip_itr_base<T...>(std::make_tuple(iters...)) {}
  };

public:
  zipper(T &...seqs)
    : begin_{ seqs.begin()... }, end_{ seqs.end()... } {}
