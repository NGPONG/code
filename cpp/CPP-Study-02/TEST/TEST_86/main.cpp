...
auto merge3(std::vector<Reward>& rewards) {
    ...
    auto cmp2 = [] (const Reward& lhs, const Reward& rhs) {
        if (lhs.GetRewardFlag(RewardFlagLevel::L1) != rhs.GetRewardFlag(RewardFlagLevel::L1))
          return lhs.GetRewardFlag(RewardFlagLevel::L1) > rhs.GetRewardFlag(RewardFlagLevel::L1);

        if (lhs.GetRewardIdx() != rhs.GetRewardIdx())
          return lhs.GetRewardIdx() < rhs.GetRewardIdx();

        // if (lhs.GetRewardId() != rhs.GetRewardId())
        //   return lhs.GetRewardId() < rhs.GetRewardId();

        // if (lhs.GetRewardFlag(RewardFlagLevel::L2) != rhs.GetRewardFlag(RewardFlagLevel::L2))
        //   return lhs.GetRewardFlag(RewardFlagLevel::L2) > rhs.GetRewardFlag(RewardFlagLevel::L2);

        // if (lhs.GetRewardGuid() != rhs.GetRewardGuid())
        //   return lhs.GetRewardGuid() < rhs.GetRewardGuid();
