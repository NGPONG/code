...
template <typename T>
using navit_itr_type = decltype(std::declval<T>().begin());
...
template <typename... T>
struct zip_itr_base:
    std::iterator<std::forward_iterator_tag, std::tuple<navit_val_type<T>...>>
{
private:
    std::tuple<navit_itr_type<T>...> iters_;
...
public:
    explicit zip_itr_base(decltype(iters_) iters) : iters_{ std::move(iters) } {}

    zip_itr_base& operator++()
    {
        inc(std::index_sequence_for<T...>{});
        return *this;
    }
...
public:
    ...
    {
        iterator(navit_itr_type<T>... iters) : zip_itr_base<T...>(std::make_tuple(iters...)) {}
    };
