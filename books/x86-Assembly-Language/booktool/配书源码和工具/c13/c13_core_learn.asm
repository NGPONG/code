...
allocate_memory:                            ;分配内存
         ...
         ; 这里是对于内存地址做强制的 4 字节对齐，因为在 x86 处理器上，内存地址的 4 字节对齐能够
         ; 加快内存的访问效率.
         ; 但是实际上，在调用该函数的时候(单指 kernel 在从硬盘中读取用户程序的时候)，
         ; 已经做了 512 字节对其，所以这里的 4 字节对其自然也就不会起作用
         mov ebx,eax
         and ebx,0xfffffffc
         add ebx,4                          ;强制对齐 
...
load_relocate_program:                      ;加载并重定位用户程序
         ...
         ; 先读取一个逻辑扇区的数据到内核的缓冲区中，以取程序的头部字段中的 [程序大小] 信息，
         ; 然后根据这个程序大小信息，内核会去分配相应字节的内存空间(4字节对齐，实际上已经 512 字节对齐了).
         ; 然后再基于这段新分配的空间，再从硬盘中读取相应的扇区填充到内存的这块空间之中
